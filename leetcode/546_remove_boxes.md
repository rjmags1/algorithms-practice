...
- To understand the code, we are essentially recursing in a way that simulates removing boxes of groups such that we ask 'if we were to remove this group `x1`, is it best to just remove this group, or remove stuff between this group `x1` and any group to the right of the same color `xk` such that the resulting `x1 + xk`, combined with the result of removing the stuff in between `x1` and `xk` yields more score.'