...
- We need to watch out for edge case where we get a remainder of `0` when we determine the amount of the current `columnNumber` that comes from the current power of `26`; this means that the letter value we are looking for (`1->a`, `2->b`, `3->c`, etc.) is `26` itself, i.e. <code>(n * 26<sup>k</sup>) % (26 * 26<sup> k - 1</sup>) == 0</code> always. 